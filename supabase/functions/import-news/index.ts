...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: existing } = await supabase
      .from("news")
      .select("id, source_url, slug, content, description")
      .in("source_url", sourceUrls);

    const existingBySource = new Map<
      string,
      { id: string; slug: string; content: string; description: string | null }
    >();
    for (const row of existing || []) {
      if (!row.source_url) continue;
      existingBySource.set(row.source_url, {
        id: row.id,
        slug: row.slug,
        content: row.content ?? "",
    ...
    for (const item of newsItems) {
      const existingRecord = existingBySource.get(item.link);
      const shouldSkip =
        !!existingRecord &&
        !!existingRecord.content &&
        existingRecord.content.trim().length >= 200;
    ...
    return new Response(
      JSON.stringify({ success: true, imported, updated, total: newsItems.length, skipped }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
